...
def test_extract_all_tools(extractor):
    with mock.patch("generate_tool_specs.dir", return_value=["MockTool"]), \
        ...
        first_init_param, second_init_param, third_init_param = sorted(init_params, key=lambda x: x['name'])
        ...
        assert third_init_param["name"] == "my_parameter_bool"
        assert third_init_param["description"] == ""
        assert third_init_param["default"] == False
        assert third_init_param["type"] == "bool"
